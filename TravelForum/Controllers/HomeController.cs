...
namespace TravelForum.Controllers
{
  public class HomeController : Controller
  {
    ...
    public ActionResult PostDetails(int postId)
    {
      ...
      List<Reply> replyList = Reply.GetRepliesByPostId(postId);
      List<City> cities = myPost.GetCities();
      List<Country> countries = new List<Country>{};
      foreach (var city in cities)
      {
        countries.Add(Country.Find(city.GetCountryId()));
      }
      Region region = Region.Find((Country.Find(cities[0].GetCountryId())).GetRegionId());
      List<Tag> allTags = Tag.GetAll();
      List<Tag> getTags = myPost.GetTags();
      model.Add("allTags", allTags);
      model.Add("post", myPost);
      model.Add("replyList", replyList);
      model.Add("region", region);
